/*! Auxin WordPress Framework - v2.4.9 - 2019-10-31
...
;(function ( $, window, document, undefined ) {
    ...
    var pluginName = "AvertaScrollAnims",
        defaults = {
            ...
            elementOrigin : 0.2,
            viewPortOrigin : [0.4, 0.6] , //[0.2, 0.6],
    ...
    function Plugin ( element, options ) {
        this.element = element;
        this.$element = $(element);
        this.$targets = this.$element.find( '.' + defaults.targets );
        // create element attribute options object
		var elementData = {},
			tempData;
		for ( var attribute in attributeDataMap ) {
            tempData = this.$targets.data(attribute);
			if ( tempData !== undefined ) {
				elementData[attributeDataMap[attribute]] = tempData;
			}
		}
    ...
    $.extend(Plugin.prototype, {
        ...
        _setStyles: function( $target, scrollValue ) {
            var delta  = this._getDelta( this.$element, this.settings.elementOrigin,  this.settings.viewPortOrigin ),
                isReverse  =  delta <= 0 ? true : false,
                styles = this._getStyle( delta ),
                effect = isReverse ? this.settings.moveOutEffect : this.settings.moveInEffect ;
            ...
            switch( effect ) {
                ...
                case 'fadeRight':
                    $target[0].style.opacity = styles.opacity;
                    $target[0].style[this._prefix + 'Transform'] = 'translateX(' + styles.yAxis + 'px)';
                    break;
                case 'fadeLeft':
                    $target[0].style.opacity = styles.opacity;
                    $target[0].style[this._prefix + 'Transform'] = 'translateX(' + -1 * styles.yAxis + 'px)';
                    break;
                case 'slideRight':
                ...
                case 'slideBottom':
                    ...
                    break;
                case 'maskTop':
                    $target[0].style.clipPath = 'inset(0 0 ' +  styles.slide + '% 0)';
                    break;
                case 'maskBottom':
                    $target[0].style.clipPath = 'inset('+  styles.slide + '% 0 0 0)';
                    break;
                case 'maskRight':
                    $target[0].style.clipPath = 'inset(0 0 0 ' +  styles.slide + '%)';
                    break;
                case 'maskLeft':
                    $target[0].style.clipPath = 'inset(0 ' +  styles.slide + '% 0 0)';
                    break;
                case 'rotateIn':
        ...
        _getStyle: function( delta ) {
            ...
            style.slide = 100 * delta;
            style.mask  = 100 * delta;
            style.rotate = this.settings.rotate * delta;
            style.scale =  this.settings.scale <= 1 ? 1 - Math.abs( ( delta * 0.5 ) ) : ( ( this.settings.scale - 1 )  * Math.abs( delta ) ) + 1  ;
            return style;   
        },
        _renderArea: function( ) {
            var renderDiv = $('<div></div>');
                renderDiv.css( {
                    position: 'fixed',
                    right: '0',
                    'background-color': 'rgba(202, 70, 70, 0.38)',
                    'z-index': '99'
                })

                if ( this.settings.type === 'vertical' ) {
                    renderDiv.css({
                        left: Array.isArray( this.settings.viewPortOrigin ) ? this.settings.viewPortOrigin[0] * 100+'vw':'0',
                        width: Array.isArray( this.settings.viewPortOrigin ) ? ( this.settings.viewPortOrigin[1] - this.settings.viewPortOrigin[0] ) * 100+'vh': this.settings.viewPortOrigin * 100+'vw',
                        height: '100vh',
                        top: 0,
                    })
                } else {
                    renderDiv.css( {
                        top: Array.isArray( this.settings.viewPortOrigin ) ? this.settings.viewPortOrigin[0] * 100+'vh':'0',
                        height: Array.isArray( this.settings.viewPortOrigin ) ? ( this.settings.viewPortOrigin[1] - this.settings.viewPortOrigin[0] ) * 100+'vh': this.settings.viewPortOrigin * 100+'vh',
                        width: '100%',
                    });
                }
                
                renderDiv.appendTo('body');
                var renderOverlay = $('<div></div>');
                renderOverlay.css( {
                    position: 'absolute',
                    top: '0',
                    left: '0',
                    right: '0',
                    height: this.settings.elementOrigin * 100+'%',
                    'background-color': 'rgba(54, 66, 210, 0.38)',
                    'z-index': '99'
                })
                renderOverlay.appendTo(this.$element);
        ...
        update: function() {
            for( var i = 0; i !== this._targetsNum; i++ ) {
                this._setStyles( this.$targets.eq(i), $window.scrollTop() );
            }
        },
